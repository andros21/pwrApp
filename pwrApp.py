...
def serve_layout(): 
    ...
    dailyStats = pd.read_sql('select * from avg', 
                             engine, 
                             index_col='index', 
                             parse_dates=['index'])
    engine.close()
    ...
    # ----------------------------------------------
    return html.Div(className='main-div', children=[
           dcc.Markdown(header, className='header-div'),
           html.Div([
               html.Div(id='serialize-df', 
                        style={'display': 'none'}, 
               ...
               dcc.Slider(id='month-slider',
                          ...
                          marks={month: datetime.date(1900, month, 1).strftime(
                              '%b') for month in range(
                                     dailyStats.index.month.min(),
                                     dailyStats.index.month.max()+1)},
                          step=None),
               dcc.Graph(id='hist-slider', className='hist-slider'),
               html.Div(className='table-slider', children=[
                          html.Div(className='plot-hover', children=[
                               dcc.Graph(id='plot-hover')]),
                          html.Div(className='mtable-slider', children=[
                               dtt.DataTable(id='mtable-slider',
                                     columns=[{'name': ' ', 'id': '1'},
                                     ...
                                     style_cell_conditional=[{
                                         'if': {'column_id': '1'},
                                         'fontWeight': 'bold'}]),
                               dcc.Markdown(id='mark-slider', className='mark-slider')
                               ])
                          ]),
               html.Div(className='table-slider', children=[
                         html.Div(className='mmark-slider', children=[
                               dcc.Markdown(id='mark1-slider', className='mark-slider')]),
                         html.Div(className='pie-slider', children=[
                               dcc.Graph(id='pie-slider')])
                         ]),
               ]),
...
def update_hist(ser, month):
    ...
    df = dailyStats[f'{year}-{str(month).zfill(2)}']
    return {
        'data': [dict(
            type='bar',
            x=df.index.day,
            y=df['kWh'],
            hovertext=[calendar.day_name[datetime.date(
                year, month, day).weekday()] for day in df.index.day],
            marker={
                'line': {
                    'width': 1.7,
                    'color': '#294763'
...
def update_mark(month):
    ...
    year = datetime.datetime.now().year
    month = month if month is not None else dmonth
    des1 = ('**Table**: stats for the ' +
            f'month of {calendar.month_name[month]} ' +
            f'of the year {year}')
    des2 = ('**Plot**: click on bars above to see ' +
            'pwr consumption history')
    des = f'> {des1}\n\n> {des2}'
    return des
...
def update_mark1(ser, month):
    ...
    des1 = ('**Pie**: Percentuage ' +
            f'of kWh for all months avaiable inside {year}')
    des2 = (f'The **total** kWh for {year} ' +
            f'are **{v}**')
    des = f'> {des1}\n\n> {des2}'
    return des
...
def update_mtable(ser, month):
    ...
    month = month if month is not None else dmonth
    dailyStats = pd.read_json(ser)
    df = dailyStats[f'{year}-{str(month).zfill(2)}']['kWh']
    label = ['max', 'min', 'mid', 'std', 'sum']
    value = np.around(
            [df.max(), df.min(), df.mean(), df.std(), df.sum()], 2)
...
def update_plot_hover(ser, month, hoverdict):
    ...
    return {
        'data': [dict(
            ...
            fill='tozeroy',
            x=np.array(
                [num for num in np.arange(0, 
                    dailyHistory['Wh'].size)]),
            y=dailyHistory['Wh'],
            marker={
                'color': '#ffce19',
            },
...
def update_pie(ser):
    ...
    colmap = ['#ffc0cb', '#ffc5b3', '#e8aaa3', '#e8a3d2',
              '#f8b3ff', '#bf9098', '#e5adb7', '#ff69b4',
              '#b23875', '#ff82c1', '#bf00bf', '#7f007f']
    label = []
    value = []
    for month in np.arange(min_month, max_month+1, 1):
        v = np.around(dailyStats[f'{year}-{str(month).zfill(2)}'].sum(), 2)[0]
